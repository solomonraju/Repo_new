...
package fl.video {
	...
	public class TimedTextManager {
		...
		flvplayback_internal var tts:Namespace;
		...
		flvplayback_internal function xmlLoadEventHandler(e:Event):void {
			try {
				if (e.type != Event.COMPLETE) {
					...
					xmlns = xml.namespace();
					tts = new Namespace(xmlns.uri + "#styling");
					...
					// handle attributes
					var clockModeAttr:XMLList = xml.@ttp::clockMode;
					if (clockModeAttr.length() > 0 && clockModeAttr[0].toString() != "media") {
						throw new VideoError(VideoError.INVALID_XML, "URL: \"" + _url + "\" Invalid value for ttp:clockMode attribute.  Only \"media\" supported, found \"" + clockModeAttr[0].toString() + "\"");
					}
